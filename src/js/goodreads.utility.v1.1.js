		...
		_booksListLanguage=function(lang){
			...
			*/
			var bookList=[],	/*selectors of books to be replaced
								...
								*/
				books,
				counterSearched=$('<div/>').appendTo(_headerReplaceBook_Counters),
				counterReplaced=$('<div/>').appendTo(_headerReplaceBook_Counters),
				total=0;

			//DEFINE DOM ELEMENTS CONTAINING BOOKS TO REPLCACE
			switch (_pageType){
...
*/
			...
			//ITERATE BOOKS TO SEARCH FOR LANGUAGE EDITION
			for (var i=0;i<bookList.length;i++){
				books=$(bookList[i].books);
				total+=books.length;
				counterSearched.html(total+' books to search');	//counter
/*TODO
una volta partite tutte le ricerche, chiudere il menù
*/
				books.each(function(){
					var book=$(this);
					//SEARCH LANGUAGE EDITION BOOK
console.log('sono qui');
/*TODO
creazione di un array con tutte le informazioni da ricercare+cache DOM
alla fine del ciclo, lanciare su n processi massimi contemporanei le ricerche di libri
	in ogni libro salvare in un data-attribute
		il fatto che è stata fatta partire la ricerca
		il risultato
			se è già un libro in italiano
			se è stato trovato\non trovato
				nel caso sia stato trovato
					sostituire link e immagine e titolo e descrizione
			se è andata in errore la ricerca (mancata connessione)

counterReplaced.html(total+' italian books found');	//counter
*/
		...
		**************************************************************************************************************/
		_bookEditionsLanguage=function(url,lang){
			/*Input parameters:
...
*/
						...
						if (nextPage.length){
							...
							//search next page
							_bookEditionsLanguage(nextPage.attr('href'),lang);
						//no more book pages
						}else{
		...
		**************************************************************************************************************/
		_menuOptionsButton=function(buttonType){
			...
			*/
			...
			/*****   FIND EDITIONS IN LANGUAGE OF THIS BOOK   ************************************************************/
...
*/
		...
		**************************************************************************************************************/
		$(document).ready(function(){
			var btn,
				language=$('<select/>',{'id':'samoLang','style':'background: #ececb6;'}),
				menu,
