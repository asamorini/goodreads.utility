...
var samoGoodreadsUtility=samoGoodreadsUtility || {'lang':'ita'};	/*possible values:
																			lang:'ita',					-> language
																			shelvesViewer:{				-> bookShelves viewer
																				excludeShelves:{			-> excludes shelves
																				...
																				shelvesNames:'',		-> replace shelves names
																												ex: "toread-|,toread_|,_|,-|"
		...
		_bookshelvesViewer=function(shelfAnalyzeIndex){
			...
			*/
			...
			_BSD.f={			//filters
				...
				NUMPAG:		_bookshelvesViewer_filterInitialize()
			};
			_BSD.f.NUMPAG.ranges=samoGoodreadsUtility['shelvesViewer'] && samoGoodreadsUtility['shelvesViewer'].filterNumPages
								 ? samoGoodreadsUtility['shelvesViewer'].filterNumPages
								 : [100,200,400,1000];
...
*/
					...
					booksList.find('#booksBody tr').each(function(){
						var el=$(this),
							...
							title=el.find('td.title a').text(),
							author=el.find('td.author a').text(),
							numPages=el.find('td.num_pages div.value').clone(),	//ex: <div class="value"><nobr>480<span class="greyText">pp</span></nobr></div>
						...
						//num pages
						numPages.find('span').remove();
						numPages=(+numPages.text().trim().replace(',','')) || 0;//ex: 480 (or 0 for undefined num pages)
						for (i=0;i<_BSD.f.NUMPAG.ranges.length;i++){
							if (_BSD.f.NUMPAG.ranges[i]>=numPages){
								_BSD.f.NUMPAG.counters[_BSD.f.NUMPAG.ranges[i]]=(_BSD.f.NUMPAG.counters[_BSD.f.NUMPAG.ranges[i]] || 0)+1;
								break;
							}
						}
		...
		_bookshelvesViewer_layoutInit=function(){
			/*Input parameters:
				?	= 
			*/
			var configDefault=samoGoodreadsUtility['shelvesViewer'] || {},
				head=$('head'),
				style=[
					'.samoHidden{',
					...
					'div.userShelf a.selected{',
						...
						'color: #fff;',
					'}',
					'#controls div.samoResetButtonContainer,',
					'#controls div.samoSearchBookContainer,',
					'#controls div.samoRenameShelves,',
					...
					'#controls div.samoExcludeShelves > div > div{',
						...
						'font-weight: normal;',
					'}',
					'#controls div.samoExcludeShelves > div > div:nth-child(3){',
						'display: block;',
					'}',
					'#controls div.samoExcludeShelves > div > div input{',
						...
						'margin-right: 5px;',
					'}',
					'#controls div.samoSearchBookContainer input.active{',
						'background: #00635d;',
						'color: #fff;',
					'}',
					...
					'#booksBody > tr td.field.actions .samoCounter{',
						...
						'color: #fff;',
					'}',
					'#shelvesSection div.samoConditionShelves button{',
						'border-radius: 5px;height: 40px;min-width: 40px;',
						'display: table-cell;',
						'background: white;',
					'}',
					'#shelvesSection div.samoConditionShelves button.selected{',
						'background: #00635d;',
						'color: white;',
					'}'
				],
			...
			//shelf names replace
			if (!controls.find('div.samoRenameShelves').length){
				controls
				.prepend(
					$('<div/>',{'class':'samoRenameShelves'})
					.append('Display shelf name')
					.append($('<div/>',{'style':'font-weight: normal;font-size:10px;'}).html('(pattern: "search|replace,...")'))
					.append(

						//REPLACE SHELVES NAME
						$('<input/>',{'type':'text','style':'width: 120px;'})
						.val(configDefault['shelvesNames'] || '')
						.change(function(){
							//iudentify rules
			...
			//search books
			if (!controls.find('div.samoSearchBookContainer').length){
				...
				.on("keydown cut paste",_debounced(150,function(){
					...
					if (value){
						...
						_BSD.fSearch.input.removeClass('active');
					}
				...
				controls
				.prepend(
					$('<div/>',{'class':'samoSearchBookContainer'})
					.append('Search books')
					.append(
						$('<div/>',{'style':'font-weight: normal;'})
						.append(_BSD.fSearch.insideAppliedFilters)
						.append($('<label/>',{'for':'samoSearchBookInsideAppliedFilters','style':'vertical-align: top;'}).html('inside applied filters'))
					)
					.append(_BSD.fSearch.input)
				)
			}
			//reset filters
			if (!controls.find('a#samoResetButton').length){
				controls
				.prepend(
					$('<div/>',{'class':'samoResetButtonContainer'})
					.append(
						$('<a/>',{'id':'samoResetButton','class':'disabled'}).html('Reset filters')
						.click(function(){
							var button=$(this);
							if (button.hasClass('disabled')){return;}
							_bookshelvesViewer_filterBooks(true);	//filter execution with "Reset all"
						})
					)
				);
			}
			...
			if (!shelvesHeader.find('div.samoConditionShelves').length){
				shelvesHeader.append(
					$('<div/>',{'class':'samoConditionShelves','style':'display: table-row;'})
					.append(
						$('<div/>',{'style':'display: table-cell;width: 40px;height: 50px;vertical-align: top;padding-right: 8px;line-height: 12px;color: #181818;font-size: 10px;font-weight: normal;'})
						.html('condition between shelves')
					)
					//"AND" button
					.append(
						$('<button/>',{'data-condition':'AND','class':'selected','style':'margin-right: 5px;'}).html('AND')
						.click(function(){
							_bookshelvesViewer_clickFilterCondition($(this));
						})
					)
					//"OR" button
					.append(
						$('<button/>',{'data-condition':'OR'}).html('OR')
						.click(function(){
							_bookshelvesViewer_clickFilterCondition($(this));
						})
					)
			...
			//execute shelves names replace
			controls.find('.samoRenameShelves input').change();
			//exclude shelves
			if (configDefault['excludeShelves']){
				for (var shelf in configDefault['excludeShelves']){
					if (shelf===_BSD.shelfAnalyzeCurrent.shelf){	//ex: "to-read"
						shelfExclude=configDefault['excludeShelves'][shelf];
			...
			//RIGHT PART: BOOKS AND OTH£R  FILTERS (years, rating, numpages, authors)
			...
			filterDOM=$('<div/>',{'style':'display:table-cell;'});
			for (i=0;i<_BSD.f.NUMPAG.ranges.length;i++){
				filterValue=_BSD.f.NUMPAG.ranges[i];
				if (filterValue===0 && !_BSD.f.NUMPAG.counters[filterValue]){continue;}	//"undefined num pages" value  display only if there are "undefined num pages" books
				filterDOM.append(
					$('<a/>',{'class':'samoFilterNumpages'+(filterValue in _BSD.f.NUMPAG.counters ? '' : ' disabled'),'data-numpages':filterValue})
					.append(
						filterValue===0
						? 'undefined'
						: (
							(_BSD.f.NUMPAG.ranges[i-1]+1)		//from ...
							+(filterValue===Number.MAX_VALUE ? '+' : ('-'+filterValue))
						)
					)
					.append(_bookshelvesViewer_layoutAddCounter(_BSD.f.NUMPAG.counters[filterValue] || 0))
					.click(function(){
						_bookshelvesViewer_clickFilter($(this));
					})
				);
			}
			_BSD.f.NUMPAG.DOManchors=filterDOM.find('a:not(.disabled)');
			_BSD.f.NUMPAG.DOMcontainer=$('<div/>',{'style':'display:table-row'})
			.append($('<div/>',{'style':'display:table-cell;min-width: 70px;'}).html('Num.pages'))
			.append(filterDOM);
			filtersContainer.append(_BSD.f.NUMPAG.DOMcontainer);
			//filter: authors
			filterValues=[];
			...
			for (var i=0;i<filterValues.length;i++){
				filterValue=filterValues[i]
				filterDOM.append(
					$('<a/>',{'class':'samoFilterAuthor','data-author':filterValue,'data-books':_BSD.f.AUTHORS.counters[filterValue]})
					.append(filterValue)
			...
			.append(
				...
				.append(
					...
					.html('Authors')
					.append(_bookshelvesViewer_layoutAddCounter(_BSD.f.AUTHORS.DOManchors.length))
					.click(function(){
						var button=$(this);
...
*/
		...
		_bookshelvesViewer_displayBooks=function(){
			...
			*/
			...
			_bookshelvesViewer_sortAuthors();	 //sort authors
		},

		//BOOKSHELVES VIEWER: display books
		_bookshelvesViewer_filterBooks=function(reset){
			/*Input parameters:
				reset	= [optional] true if we want to reset all previous applied filters
			*/
			var book,
				title,shelves,dateRead,dateAdded,year,rating,numpages,author,
				filters={	//filters applied
					...
					numpages:_BSD.f.NUMPAG.DOManchors.filter('.selected'),
					authors:_BSD.f.AUTHORS.DOManchors.filter('.selected')
				},
				filterSearch={
					value:					_BSD.fSearch.input.val().toLowerCase(),
					insideAppliedFilters:	_BSD.fSearch.insideAppliedFilters.prop('checked')
				},
				ok,okNumber=0,atLeastOneFound,
				filterYear,filterShelves,filterRating,filterNumpages,filterAuthor,
				el,
				shelf,foundNumber,numPagesRangeSelected=[],
				counters={};

			//initialize counters for this filter
			for (var filter in filters){
				counters[filter]={};
			}
			//RESET FILTER
			if (reset || (filterSearch.value && !filterSearch.insideAppliedFilters)){
				//remove all previously selected filters
				for (var f in filters){
					if (reset){
						filters[f].removeClass('selected');
					}
					filters[f]=$();	//reset variable pointing to active filters
				}
			}
			atLeastOneFound=false;
			for (var f in filters){
				if (filters[f].length){atLeastOneFound=true;break;}
			}
			if (atLeastOneFound){
				$('a#samoResetButton').removeClass('disabled');
			}else{
				$('a#samoResetButton').addClass('disabled');
			}

			//retrieve ranges for numpages filter
			for (var k=0;k<filters.numpages.length;k++){
				numPagesRangeSelected.push($(filters.numpages[k]).data('numpages'));	//ex: 200
			}
...
//				dateAdded=book.data('dateAdded');
				...
				//transform numpages of this book (ex: 140) to range (ex: 200)
				for (z=0;z<_BSD.f.NUMPAG.ranges.length;z++){	//for every possible ranges; ex: [0,100,200,400,1000,Number.MAX_VALUE]
					//max num pages for this ranve; ex: 140p in range 100-200
					if (_BSD.f.NUMPAG.ranges[z]>=numpages){
						numpages=_BSD.f.NUMPAG.ranges[z];	//ex: 200
						break;
					}
				}

				ok=true;
				if (!filterSearch.value || filterSearch.value && filterSearch.insideAppliedFilters){
					...
					if (ok && filters.shelves.length){
						...
						for (var k=0;k<filters.shelves.length;k++){
							...
							if (shelves.indexOf(filterShelves)===-1){
								if (_BSD.shelvesCondition==='AND'){
									ok=false;
									break;
								}
							}else{
								if (_BSD.shelvesCondition==='OR'){atLeastOneFound=true;}
							}
						}
						if (_BSD.shelvesCondition==='OR'){
					...
					//NUMPAGES filter
					//check numpages filters
					if (ok && numPagesRangeSelected.length){
						atLeastOneFound=false;
						for (var k=0;k<numPagesRangeSelected.length;k++){	//for "to" numpages filter selected; ex: [200,400]
							if (numpages===numPagesRangeSelected[k]){
								atLeastOneFound=true;
								break;
							}
						}
				...
				//SEARCH INPUT filter
				if (ok && filterSearch.value){
					if (author.toLowerCase().indexOf(filterSearch.value)===-1
					&& title.toLowerCase().indexOf(filterSearch.value)===-1){
						ok=false;
					}
				}
				...
				if (ok){
					...
					counters['numpages'][numpages]=(counters['numpages'][numpages] || 0)+1;
			...
			//NUMPAGES
			for (var i=0;i<_BSD.f.NUMPAG.DOManchors.length;i++){
				el=$(_BSD.f.NUMPAG.DOManchors[i]);
				numpages=el.data('numpages');
				okNumber=counters['numpages'][numpages] || 0;
				//update numpages counter
				el.find('span').html(okNumber);
				if (okNumber){
					el.css({'opacity':'1'});
				}else{
			...
			for (var i=0;i<_BSD.f.AUTHORS.DOManchors.length;i++){
				...
				if (okNumber){
					el.show();
					el.data('books',okNumber);	//save counter also on <a ...> for sorting purpose
					foundNumber++;
				}else{
					el.hide();
				}
			}
			_BSD.f.AUTHORS.DOMcontainer.find('#samoAuthorsButton span').html(foundNumber);
			_bookshelvesViewer_sortAuthors();	 //sort authors
		},

		//BOOKSHELVES VIEWER: sort authors by books number\name
		_bookshelvesViewer_sortAuthors=function(n){
			var authorsSorted=_BSD.f.AUTHORS.DOManchors.filter('[data-books]');
			authorsSorted.sort(function(a,b){
				var a=$(a),
					...
					aBooks=+a.data('books'),
					bBooks=+b.data('books');
	    		//sorting by books number descending
				if(aBooks > bBooks){return -1;}
				if(aBooks < bBooks){return 1;}
	    		//sorting by author name alphabetically
				if(aName > bName){return 1;}
				if(aName < bName){return -1;}
				return 0;
			});
    		authorsSorted.detach().appendTo('.samoFilterAuthorsContainer');
		},
		...
		//BOOKSHELVES VIEWER: click function on AND-OR filter condition
		_bookshelvesViewer_clickFilterCondition=function(el){
			/*Input parameters:
				el	= button clicked
			*/
			if (el.hasClass('selected')){return;}
			//change condition between shelves (from "AND" to "OR" or viceversa)
			_BSD.shelvesCondition=el.data('condition');	//ex: "AND", "OR"
			el.parent().find('button').each(function(index,button){
				button=$(button);
				if (button.data('condition')===_BSD.shelvesCondition){
					button.addClass('selected');
				}else{
					button.removeClass('selected');
				}
		...
		//BOOKSHELVES VIEWER: click function on shelf\year\AND-OR filter
...
*/
			...
			case 'bookshelvesViewer':
				...
				for (var i=0;i<_BSD.shelvesAnalyze.length;i++){
					...
					//default value
					if (samoGoodreadsUtility['shelvesViewer'] && samoGoodreadsUtility['shelvesViewer']['analyzeShelfDefault']){	//default analyze shelf setted; ex: "to-read"
						if (_BSD.shelvesAnalyze[i].shelf===samoGoodreadsUtility['shelvesViewer']['analyzeShelfDefault']){
							_BSD_shelfAnalyze.val(_BSD.shelvesAnalyze[i].name);
						}
					}
				}
				left
