		...
		_booksListLanguage=function(lang){
			...
			*/
			var bookList=[],	/*selectors of books to be replaced
				...
				counterReplaced=$('<div/>',{'style':'color:red'}).appendTo(_headerReplaceBook_Counters),
				total=0,totalSearched=0,totalReplaced=0,
				completeSearch=function(){
					//all books are searched
					if (total===totalSearched){
						counterReplaced.html(totalReplaced+' replaced');
...
*/
			...
			if (!bookList.length){return completeSearch();}
...
/*TODO
					if (book.data('samoLangEdition')==='SEARCH_COMPLETED'){return true;}
*/

					//SEARCH LANGUAGE EDITION FOR THIS BOOK
					console.log(_logPrefix+'REPLACE BOOK LIST IN LANGUAGE; searching book '+totalSearched+' '+url);
					//save data
					book.data({
					...
					$.ajax({
						...
						success:function(data){
							...
							_bookEditionsLanguage(urlEditions,lang,{
								...
								callback:function(found,el,bookToReplace){
									...
									*/
									var info,
										...
										img;
									totalSearched++;
									bookToReplace.data('samoLangEdition','SEARCH_COMPLETED');
									...
									if (found){
										...
										if (info.replace.links){
											bookToReplace.find(info.replace.links).prop('href',link.attr('href'));
										}
										if (info.replace.title){
											bookToReplace.find(info.replace.title).html(link.html());
										}
										if (info.replace.img){
